...
def train_one(model_name: str, bundle: TrainBundle, target: str, genes: list[str]) -> object:
    ...
    if model_name == "catboost_optuna":
        from final_train.training.catboost_trainer import train_catboost_optuna

        return train_catboost_optuna(arr, out_dir)
    from final_train.training.torch_trainers import train_resnet_model, train_tabm_model
...
def predict_one(model_name: str, artifact, x: np.ndarray) -> np.ndarray:
    ...
    from final_train.training.torch_trainers import predict_resnet_model, predict_tabm_model
