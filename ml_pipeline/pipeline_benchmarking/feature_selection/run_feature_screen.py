"""Stage01: feature selection screen."""
...
def screen_modality(X, y, feature_names: list[str], method: str) -> tuple[list[str], dict]:
    import numpy as np
    ...
    elif method == "method_d_elasticnet":
    ...
    elif method == "method_e_spearman":
        chosen_idx, stage_meta = select_spearman_only(pool_idx, rhos)
    else:
...
def run_method(method: str, targets: list[str], bulk_x, bulk_y, sc_x, sc_y, feature_names: list[str]) -> None:
    ...
    for i, target in enumerate(targets, start=1):
        ...
        try:
            ...
            final_genes, union_meta = union_sc_priority(sc_genes, bulk_genes, MAX_FINAL_FEATURES)
            features_out[target] = final_genes
            details[target] = {
                "n_bulk": len(bulk_genes),
                "n_sc": len(sc_genes),
                "n_union_raw": len(set(bulk_genes) | set(sc_genes)),
            ...
            log(
                ...
                f"final={len(final_genes)} (sc_first cap {MAX_FINAL_FEATURES}) "
    ...
    rows = [
        {
            ...
            "n_sc_in_final": d.get("union_meta", {}).get("n_sc_in_union"),
...
def main() -> None:
    ...
    log("=== Stage01 v2: feature selection screen ===")
    ...
    config = {
        ...
        "union_policy": "sc_first_then_bulk_only",
        "adaptive_second_stage": True,
