"""Load stage00 split matrices for stage01."""
...
def load_sc_val_combo() -> tuple[pd.DataFrame, pd.DataFrame]:
    x_k1 = pd.read_parquet(SPLITS / "sc_k1" / "X_val.parquet")
    y_k1 = pd.read_parquet(SPLITS / "sc_k1" / "Y_val.parquet")
    x_pb = pd.read_parquet(SPLITS / "sc_pb" / "X_val.parquet")
    y_pb = pd.read_parquet(SPLITS / "sc_pb" / "Y_val.parquet")
    m = _k2_mask(x_pb.index)
    x = pd.concat([x_k1, x_pb.loc[m]], axis=0)
    y = pd.concat([y_k1, y_pb.loc[m]], axis=0)
    return x, y
