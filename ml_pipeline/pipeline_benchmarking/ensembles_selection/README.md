...
2. **Weighted Blending:** Linear combinations of model predictions. 
   $$\text{Example (2 models): } \hat{y} = \alpha \cdot \text{pred}_{\text{model}_1} + (1 - \alpha) \cdot \text{pred}_{\text{model}_2}$$
...
The ensembles successfully pushed the performance boundary beyond any individual model. When evaluated by **median $R^2$**, the top-performing combinations were:
